...
> 状态：调研记录 · 2026-08-10；结论已被桌面方案吸收  
> 背景：文档以 Markdown 为真相源（见 [04](./04-文档存储原则.md)）。桌面主区用墨览（Vditor IR），见 [00](./00-产品重定位.md)。本文保留对照过程。

## 1. 选型约束（DesignWeave）

- 落盘必须是 **可 Git / 可旁路打开的 Markdown**（优先 GFM）
- 宿主：浏览器（本机服务器 Web UI）/ VS Code webview（同一套 `MolanEditor`）
- 用户：产品经理 / 软件设计师为主，中文优先；不要求人人会写 md 语法

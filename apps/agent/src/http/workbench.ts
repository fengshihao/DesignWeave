...
export function registerWorkbenchRoutes(app: Express): void {
  for (const base of PROJECT_API_BASES) {
    mountWorkbench(app, base);
  }
}

function mountWorkbench(app: Express, base: string): void {
  app.post(`${base}/:id/lock/claim`, (req, res) => {
    try {
      res.json(
  ...
  app.post(`${base}/:id/lock/heartbeat`, (req, res) => {
    try {
      res.json(
        heartbeatProjectLock(
          actorOf(req),
  ...
  app.post(`${base}/:id/lock/release`, (req, res) => {
    try {
      res.json(
        releaseProjectLock(
          actorOf(req),
  ...
  app.post(`${base}/:id/lock/force-release`, (req, res) => {
    try {
  ...
  app.get(`${base}/:id/chat-sessions`, (req, res) => {
    try {
      res.json(
  ...
  app.post(`${base}/:id/chat-sessions`, (req, res) => {
    try {
  ...
  app.get(`${base}/:id/chat-sessions/open`, (req, res) => {
    try {
  ...
  app.get(`${base}/:id/chat-sessions/:sid`, (req, res) => {
    try {
  ...
  app.post(`${base}/:id/chat-sessions/:sid/close`, (req, res) => {
    try {
  ...
  app.get(`${base}/:id/runs/current`, (req, res) => {
    try {
  ...
  app.get(`${base}/:id/runs/:runId/events`, (req, res) => {
    try {
  ...
  app.get(`${base}/:id/runs/:runId/stream`, (req, res) => {
    try {
      const run = assertRunOwnedByUser(req.params.runId, req.user!.id, req.params.id);
      const after = Number(req.query.after || 0) || 0;
  ...
  app.post(`${base}/:id/runs/:runId/cancel`, (req, res) => {
    try {
      res.json(
        cancelProjectRun(
          actorOf(req),
  ...
  app.post(`${base}/:id/questions`, (req, res) => {
    try {
